-- 본당: 매주 일요일 09:00–10:00 주일 1부예배 × 52주 (승인완료)
-- Supabase Dashboard > SQL Editor에서 실행하세요.
-- 시간대: Pacific/Auckland (뉴질랜드)
-- 시작: 2026-08-09(일)부터 52주
--
-- IMPORTANT: generate_series(date, date, interval) yields timestamptz.
-- Always cast to date + timestamp WITHOUT time zone before AT TIME ZONE,
-- otherwise Sunday 09:00 NZ becomes Monday 09:00 NZ.

do $$
declare
  v_user_id uuid;
  v_contact_name text;
  v_contact_phone text;
  v_main uuid;
  v_group uuid := gen_random_uuid();
  v_first_sunday date := date '2026-08-09';
  v_last_sunday date := date '2026-08-09' + interval '51 weeks';
  v_inserted integer;
begin
  select id, coalesce(nullif(full_name, ''), email), phone
  into v_user_id, v_contact_name, v_contact_phone
  from public.profiles
  where role = 'admin'
  order by updated_at desc nulls last, created_at desc
  limit 1;

  if v_user_id is null then
    select id, coalesce(nullif(full_name, ''), email), phone
    into v_user_id, v_contact_name, v_contact_phone
    from public.profiles
    order by created_at
    limit 1;
  end if;

  if v_user_id is null then
    raise exception 'profiles에 사용자가 없습니다. 먼저 회원가입/로그인을 해 주세요.';
  end if;

  select id into v_main from public.rooms where code = 'main-hall' limit 1;
  if v_main is null then
    raise exception '본당(main-hall)을 찾을 수 없습니다. rooms 시드를 먼저 적용하세요.';
  end if;

  alter table public.reservations disable trigger reservations_block_holidays;
  alter table public.reservations disable trigger reservations_enforce_status;

  -- 같은 제목의 기존 1부 예배 시드 정리 (잘못된 월요일 슬롯 포함, 재실행 가능)
  delete from public.reservation_histories
  where reservation_id in (
    select r.id
    from public.reservations r
    where r.room_id = v_main
      and r.title in ('주일 1부예배', '주일 오전1부 예배')
  );

  begin
    delete from public.reservation_email_logs
    where reservation_id in (
      select r.id
      from public.reservations r
      where r.room_id = v_main
        and r.title in ('주일 1부예배', '주일 오전1부 예배')
    );
  exception
    when undefined_table then null;
  end;

  delete from public.reservations r
  where r.room_id = v_main
    and r.title in ('주일 1부예배', '주일 오전1부 예배');

  insert into public.reservations (
    room_id, user_id, title, purpose, contact_name, contact_phone,
    start_at, end_at, status, notes, approved_by, approved_at, recurrence_group_id
  )
  select
    v_main,
    v_user_id,
    '주일 1부예배',
    '주일 1부예배',
    v_contact_name,
    v_contact_phone,
    ((d::date + time '09:00')::timestamp without time zone at time zone 'Pacific/Auckland'),
    ((d::date + time '10:00')::timestamp without time zone at time zone 'Pacific/Auckland'),
    'approved',
    '정기 예배 (자동 승인)',
    v_user_id,
    now(),
    v_group
  from generate_series(v_first_sunday, v_last_sunday, interval '7 days') as g(d);

  get diagnostics v_inserted = row_count;

  alter table public.reservations enable trigger reservations_enforce_status;
  alter table public.reservations enable trigger reservations_block_holidays;

  if v_inserted <> 52 then
    raise exception '예상 52건이 아닌 %건이 삽입되었습니다.', v_inserted;
  end if;

  raise notice '본당 주일 1부예배 %건 등록 완료 (승인완료, % ~ %)',
    v_inserted, v_first_sunday, v_last_sunday;
end;
$$;

-- 확인: first_sunday / last_sunday 가 일요일이어야 함 (dow=0)
select
  count(*) as cnt,
  min((start_at at time zone 'Pacific/Auckland')::date) as first_sunday,
  max((start_at at time zone 'Pacific/Auckland')::date) as last_sunday,
  extract(dow from min((start_at at time zone 'Pacific/Auckland')::date)) as first_dow,
  status
from public.reservations
where title = '주일 1부예배'
group by status;
